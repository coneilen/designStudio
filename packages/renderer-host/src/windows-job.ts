...
export interface Jobs {
  create(name: string): OwnedJob;
  joinCurrent(name: string): void;
}
...
async function load(): Promise<Jobs> {
  ...
  const checkFlags = (handle: Handle): number => {
    const info = Buffer.alloc(144);
    if (!query(handle, 9, info, info.length, null))
      throw failure("limits query");
    if (
      info.readUInt32LE(16) !== FLAGS ||
      info.readUInt32LE(40) !== MAX_PROCESSES
    )
      throw new HostBoundaryError(
        "POLICY_FAILED",
  ...
  return {
    create(name) {
      ...
      try {
        ...
        info.writeUInt32LE(MAX_PROCESSES, 40);
      ...
      return {
        flags() {
          ensureOpen();
          return checkFlags(handle);
        },
        members() {
          ensureOpen();
          const info = Buffer.alloc(8 + MAX_PROCESSES * 8);
          if (!query(handle, 3, info, info.length, null))
            throw failure("members query");
          const count = info.readUInt32LE(4);
          if (count > MAX_PROCESSES || info.readUInt32LE(0) !== count)
            throw new HostBoundaryError(
              "INTERRUPTED",
