...
it("preserves exact translated text with declared synthetic font, and blocks missing fonts", async () => {
  const input = fixture((root) => {
    root.children = [
      {
        id: "1:2",
  ...
  expect(text.appearance).toEqual({ opacity: 0.75, clip: { kind: "bounds" } });
  const layout = await layoutDesign(root, 240, 160, async () => ({
    width: 90,
    height: 20,
  }));
