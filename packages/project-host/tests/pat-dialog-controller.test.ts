...
windows(
  "synthetic process adapter retains project ownership until actual close and rejects copied project proof",
  async () => {
    ...
    try {
      await withCaptureInstallation(async (installation) => {
        ...
        try {
          expect(() =>
            startCapturePatDialog({ ...project }, new AbortController().signal),
          ).toThrow(/live native/);
          ...
          for (let count = 0; count < 100 && !sentClosed; count++) await tick();
          await peer;
