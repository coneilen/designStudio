...
import { ownTestWork } from "./test-scope.js";

for (const refresh of ["unavailable", "rejected"] as const) {
  describe(`owned ${refresh} journal-refresh fixture`, () => {
    let prepared:
      | {
          f: Awaited<ReturnType<typeof fixture>>;
          observed: ReturnType<typeof observeSelectedTest>;
          settled: () => void;
        }
      | undefined;
    ...
    beforeEach(async ({ signal, task }) => {
      ...
      try {
        prepared = await ownTestWork(async () => ({
          f: await fixture({ observation: observed }),
          observed,
          settled,
        }))();
      } catch (error) {
    ...
    test(`partial stage receipt survives ${refresh} journal refresh and cannot complete`, async () => {
      if (!prepared) throw new Error("Missing owned journal-refresh fixture.");
      const { f, observed, settled } = prepared;
      try {
        ...
        expect(validateContract("ProviderOutcome", original).success).toBe(
          true,
        );
        expect(validateContract("ProviderOutcome", received).success).toBe(
          true,
        );
        ...
        expect(
          value(await f.store.jobs.getStages("job-work", f.context()))[0]
            ?.staged,
